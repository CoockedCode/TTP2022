<?php
    ...
    function save_to_db($project_num, $client, $machine_type, $priority, $start_date, $planned_end_date,
        ...
        // if($stmt->execute()){
        //     array_push($list_html, array("notice"=>"Salvestamine õnnestus!", "type"=>"success"));
        // } else {
        //     array_push($list_html, array("notice"=>'Tekkis viga: ' .$conn->error, "type"=>"error");)
        // }
        $stmt->execute();
        $stmt->close();
        // transpordi firma salvestamine
        if($project_arrived_by == "transpordifirma"){
            $stmt = $conn->prepare("INSERT INTO transport(id, projekt_id, firma_id, suund) VALUES (NULL,(SELECT MAX(id) FROM projekt),?,1)");
            $stmt->bind_param("i", $project_arrived_transport);
            $stmt->execute();
            $stmt->close();
        }
        if($project_return_by == "transpordifirma"){
            $stmt = $conn->prepare("INSERT INTO transport(id, projekt_id, firma_id, suund) VALUES (NULL,(SELECT MAX(id) FROM projekt),?,2)");
